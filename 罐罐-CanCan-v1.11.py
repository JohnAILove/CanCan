...
class CanCanApp(tk.Tk):
    ...
    def open_message_editor(self, event):
        ...
        try:
            ...
            if hasattr(self, 'message_editor') and self.message_editor.winfo_exists():
                return

            self.message_editor = tk.Toplevel(self)
            self.message_editor.title("編輯訊息")

            title_label = ttk.Label(self.message_editor, text="標題：")
            title_label.pack()

            title_entry = ttk.Entry(self.message_editor)
            title_entry.insert(0, selected_message_title)
            title_entry.pack()

            content_label = ttk.Label(self.message_editor, text="內容：")
            content_label.pack()

            content_entry = tk.Text(self.message_editor, wrap=tk.WORD)
            content_entry.insert(tk.END, message_content)
            content_entry.pack(expand=True, fill=tk.BOTH)
            save_button = ttk.Button(self.message_editor, text="保存",
                                     command=lambda: self.save_message_changes(messages_listbox, content_entry,
                                                                               title_entry, index, self.message_editor))

            save_button.pack()
            self.message_editor.protocol("WM_DELETE_WINDOW", lambda: self.close_message_editor(selected_message_title, self.message_editor))
    ...
    def save_message_changes(self, messages_listbox, content_entry, title_entry, index, message_editor):
        ...
        self.update_messages_listbox(messages_listbox)
        self.save_data()
        self.close_message_editor(new_title, message_editor)
    ...
    def close_message_editor(self, message_title, message_editor):
        message_editor.destroy()
        delattr(self, 'message_editor')
